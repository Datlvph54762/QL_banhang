...
@section('content')
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('admin.products.productVariants.index', $productVariant->product_id) }}"
                class="btn btn-sm btn-secondary">
                <i class="fas fa-arrow-left"></i> Quay lại
            </a>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.products.productVariants.update', $productVariant->id) }}" method="POST"
                    ...
                    <div class="col-6 mb-3 ps-2">
                        ...
                        <select name="size_id" id="size_id" class="form-select form-control">
                            @foreach($sizes as $size)
                                <option value="{{ $size->id }}" @selected($size->id == $productVariant->size_id)>
                                    {{ $size->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-12 mb-3">
                    ...
                    <input type="text" name="price" class="form-control" value="{{ $productVariant->price }}">
                    @error('price')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="col-12 mb-3">
                    ...
                    <input type="text" name="sale" class="form-control" value="{{ $productVariant->sale }}">
                </div>
